...
| **BitLocker must stay off — and it turns itself ON** | This is the risk that can cost more than a failed boot, and it has already happened once on this host: `Storage` was plain NTFS in the morning and BitLocker by the afternoon, after a single boot with the drive linked. Windows 11 enables device encryption **by itself** when it sees a TPM 2.0 and Secure Boot, both of which this VM provides on purpose. If it does that to C: while running as the guest, the key is sealed to the **virtual** TPM — and the next **bare-metal** boot then demands a recovery key for its own system drive. Before the first guest boot, confirm the recovery key is in the Microsoft account (aka.ms/myrecoverykey); after it, re-check the volumes and turn encryption off again if it came back. A stronger block, set from inside Windows, is `HKLM\SYSTEM\CurrentControlSet\Control\BitLocker\PreventDeviceEncryption = 1` (DWORD). Check from the host without mounting anything: a BitLocker volume reads `-FVE-FS-` at offset 3 of its first sector, a plain one reads `NTFS    `. |
