...
updated: 2026-07-25T22:30:00Z
resolved: 2026-07-25
resolved_by: 04-06
resolution: >
  The handed-off fix shipped in commit 520f6a7 "fix(04-06): guard empty-submit
  trigger with ignore_empty_input (FIX-02)". Verified present in the working
  tree: hypr/.config/hypr/hyprlock.conf:21 sets `ignore_empty_input = true`,
  with a comment at line 13 citing "FIX-02 gap closure, UAT Test 2 / D-23".
  This removes the trigger — ENTER on an empty field no longer opens a PAM
  round, so there is no in-flight window during which hyprlock 0.9.5 drops
  keystrokes. Session was left at `diagnosed` only because the debug goal was
  find_root_cause_only; the fix was delivered by plan-phase --gaps and never
  reflected back here. Reconciled at v2.0 milestone close.
---
