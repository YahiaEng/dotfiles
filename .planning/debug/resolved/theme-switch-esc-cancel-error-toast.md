...
resolved_by: 05-05
resolution: >
  The handed-off fix shipped in commit 200e7e0 "feat(05-05): three-way
  exit-code branch in theme-switch.sh [WR-04]". Verified present in the
  working tree: hypr/.config/hypr/scripts/theme-switch.sh:53 branches on
  `(( rc == 130 ))` and returns `exit 0` for a user cancel, with a comment at
  lines 44-47 citing this debug session by path and naming walker 2.16.2's
  exit-130 cancel convention. The error toast now fires only on genuine
  failures, not on Esc / click-outside / empty-line Return. Session was left
  at `diagnosed` only because the debug goal was find_root_cause_only; the fix
  was delivered by plan-phase --gaps and never reflected back here.
