...
- [x] **QBAR-08**: The hidden bar reveals on pointer hover **and** on holding Super *(named addition)*
...
- [x] **GATE-02**: Every phase passes a human render-and-look gate; no phase closes with its replacement judged worse than what it replaced, and no old package is deleted before that judgment — **PASSED at Phase 18.1 (2026-08-12).** First run FAILED (2026-08-11) on four operator-reported defects; all four were root-caused and fixed, followed by three further rounds of operator review (upstream-reference correction, spacing/pill geometry, drawer animation). Re-run approved by the operator: the restored QML bar is judged at least as good as the Athena waybar. Per D-23 this UNBLOCKS Phase 18 plans 18-19 (gate re-run) and 18-20 (waybar package retirement); waybar remains installed until 18-20 runs. Evidence: `18.1-07-SUMMARY.md`, `18.1-VERIFICATION.md`, `ATHENA-UPSTREAM-SPEC.md`.
