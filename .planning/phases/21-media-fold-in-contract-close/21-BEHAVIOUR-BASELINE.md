...
| C-11 | Per-track seekability latch (survives a transient zero-length report mid-track) | **CLOSED, 21-07 (D-21-09..D-21-12 plan).** `MediaBackend.qml` now carries a `_trackKeyOf`/`_updateSeekLatch`/`_seekLatchTrackKey`/`_seekLatchSeekable`/`_seekLatchLength` group ported from `lib/media.ts:17-68`'s own `trackKeyOf`/`updateSeekLatch` pair: once a track (keyed on `player+title+artist`) is confirmed seekable (native `canSeek === true` AND a positive length observed), `canSeek`/`lengthSeconds` hold that confirmation across a later transient zero-length/false-canSeek report, resetting only on a genuine track-identity change or the player disappearing. Wired off the player's own `canSeekChanged`/`lengthChanged`/`lengthSupportedChanged`/`trackTitleChanged`/`trackArtistChanged` signals (confirmed present on the installed `quickshell-service-mpris.qmltypes`) plus `onActivePlayerChanged`/`Component.onCompleted`, mirroring the file's existing album-art-resolve wiring shape rather than inventing a new one. | SATISFIED | Built rather than re-verified-and-waived, per D-21-11's build-before-delete rule — the live transient condition this session's Provenance recorded (`length:0`,`can_seek:false` on the real Firefox/YouTube track) is exactly what the latch now survives. Live re-verification (a real seek, watched for a seek-row flicker) is still owed to D-21-20's combined render gate, same as every other visual claim in this phase — this row's SATISFIED status is about the mechanism now existing and being wired correctly, not a substitute for that gate. |
...
(C-06, C-14), 0 unclassified, 0 GAP. C-11 (the per-track seekability latch) closed
2026-08-16 under Plan 21-07: the latch mechanism now exists in `MediaBackend.qml`, ported
from the retiring card's own `trackKeyOf`/`updateSeekLatch` shape, per D-21-11's
build-before-delete rule. Every row carries a real status derived from the current tree —
none is silently dropped, and none is written off as a lost capability.

```
Parity: 16/16 SATISFIED, 0 GAP
```
