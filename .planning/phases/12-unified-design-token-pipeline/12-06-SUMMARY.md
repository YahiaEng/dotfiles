...
- **`theme-stress-test`'s strict per-switch `theme-doctor` gate (D-66) cannot pass in this dev environment**, regardless of this plan's changes: the sole untracked file (`vscodium/.local/share/applications/Vampire Survivors.desktop`) — pre-existing, out of scope, documented in `deferred-items.md` and `STATE.md` since before Phase 12 began — fails `theme-doctor`'s git-clean check on every invocation. This blocks a genuine unattended 10/10 run of the REAL, committed `theme-stress-test` regardless of any code in this plan. D-17's own logic was instead verified via a scratch, never-committed patched copy that bypassed only that one pre-existing failure (see "Known Limitations" below) — recorded as an `unrun-verify` in `.planning/WINDOWS.md` (entry #9) so it stays visible until the untracked file is resolved by its owner and the real script can be re-run.

## Known Limitations

- **D-17's live re-colour assertion has NOT been proven via the real, committed `theme-stress-test` end-to-end (10/10 switches).** It was verified via a scratch-patched, never-committed copy of the script (identical except `check_theme_doctor` treated the one known pre-existing git-clean failure as non-blocking) across two runs: 6 consecutive real rsync-based switches passed cleanly in one run, 4 in a second run before unrelated leftover-process interference corrupted shared theme state mid-run (documented above, not a defect in D-17's own logic — every switch D-17's checks actually reached returned PASS for both the live re-colour match and the PID-unchanged assertion). The scratch copy was deleted, never committed, and is not part of this plan's shipped `theme-stress-test`. **Action for whoever resolves the pre-existing untracked file:** re-run `~/.config/theme-engine/theme-stress-test` for a genuine unattended 10/10 confirmation. Tracked in `.planning/WINDOWS.md` entry #9.
