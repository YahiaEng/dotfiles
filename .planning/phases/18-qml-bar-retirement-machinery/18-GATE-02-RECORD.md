...
|---|---|---|---|---|---|
| A.1 | Every capsule is a fully rounded, discrete pill with a visible gap to its neighbors — never one continuous bar (matches D-18-09, contradicts athena's own `"spacing": 0`, which this redesign deliberately supersedes). | athena | Look at the horizontal bar, count the capsules, and confirm each has a visibly rounded left and right end with a visible background gap to its neighbour. The failure this catches is a run of capsules reading as one continuous bar, which is what athena's own `"spacing": 0` produced and what D-18-09 deliberately supersedes. | **PASS** | **Iteration 3, live gesture.** Counted the horizontal bar's capsules: each rendered as a fully rounded pill with a visible background gap to its neighbour — no run of capsules read as one continuous bar. **Known-state note:** the clock pill carries a measured ~6px vertical/horizontal offset relative to its cell (`x=8.0 y=1176.0` vs. cell `x=2.0 y=1182.0`), named in this iteration's `## Build Under Test` block before the sitting — a fix was measured working but reverted for a `PopoutTrigger.qml:40` binding loop, so the offset is still live. The operator judged the criterion sentence itself (discrete rounded capsules with visible gaps, never one continuous bar) met regardless: the pill is still a single, separate, rounded capsule with its own gap to its neighbours — the offset is a positioning residual, not a fusion into a continuous bar. |
| A.2 | The bar floats clear of the screen edge with a visible margin on every visible side — never edge-flush. | athena | With a maximised window behind it, confirm a strip of desktop is visible above the bar and beyond both of its ends. The floating posture is D-18-08's; the two outer offsets are UI-SPEC's only two grid exemptions and exist to land the bar where the retired one sat. | **PASS** | **Iteration 3, live gesture.** With a maximised window behind the bar, a strip of desktop remained visible above the bar and beyond both of its ends — the bar floats clear of the screen edge on every visible side, never edge-flush. |
| A.3 | Workspace indicators show live per-app window icons in the `{icon} {windows}` shape — never dots, pills, or bare numbers. | athena | Open a terminal on one workspace and a browser on another, then confirm each occupied slot renders that application's glyph and each empty slot renders a numeral only (D-18-02, D-18-12). Name the specific failure mode: a Material Symbols ligature that does not exist renders as its own name in plain text, so a slot showing a word instead of a glyph is a fail, not a styling nit. | **PASS** | **Iteration 3, live gesture.** With a terminal on one workspace and a browser on another, the occupied slots rendered each application's glyph and empty slots rendered a numeral only — no Material Symbols ligature rendered as its own name in plain text. **Known-state note:** workspace numerals sit a measured ~3px left of their slot centre (numeral centre `19.0` vs. slot centre `22.0`), named in this iteration's `## Build Under Test` block before the sitting and left unfixed. The operator judged the criterion sentence itself (live per-app icons for occupied, numerals for empty, never dots/pills/bare numbers) met regardless: the shape is correct — glyph vs. numeral, never a dot or bare pill — the offset is a positioning residual within an already-correct shape. |
| A.4 | Switching the live theme re-colors every bar element (capsule fills, active-state tints, the `GradientBorder` rim) within one crossfade, with zero literal-hex flash anywhere. | athena | Switch the live theme and watch the bar through the transition, confirming capsule fills, the active-workspace tint and the `GradientBorder` rim all re-colour within one crossfade. Name the specific artefact: `Colours.qml`'s nineteen fallback defaults are magenta, so a magenta flash means a palette read failed rather than that an animation was slow. | **PASS** | **Iteration 3, live gesture.** Switched the live theme and watched the bar through the transition: capsule fills, the active-workspace tint, and the `GradientBorder` rim all re-coloured within one crossfade — no magenta flash observed, so no palette read failed. |
| A.5 | The bar reads as *denser* than end-4/Caelestia, not simplified toward them: both drawers (launcher + settings), the always-visible tray, and all four extras (power, gaming toggle, bell, updates+idle) are visibly present, not folded away or deferred — this is the explicit check against the standing instruction not to simplify toward the reference shells. | athena | Without clicking anything, confirm all of these are simultaneously visible: the launcher drawer trigger, the settings drawer trigger, the tray icon row, and the four extras — power, gaming-mode toggle, notification bell, and the updates count with the idle inhibitor (D-18-01, D-18-03, D-18-04). At zero pending updates the updates item rendering nothing is the specified empty state, not a miss. This row is the explicit check against simplifying toward end-4/Caelestia. | **PASS** | **Iteration 3, live gesture.** Without clicking anything, the launcher drawer trigger, the settings drawer trigger, the tray icon row, and all four extras (power, gaming-mode toggle, notification bell, and the updates count with the idle inhibitor) were simultaneously visible. Zero pending updates rendered nothing, the specified empty state, not a miss. Bar reads as dense, not simplified toward end-4/Caelestia. **Post-gate correction:** the criterion's "always-visible tray" clause and the gesture's "tray icon row" are stale — the tray was withdrawn by 18.1-04 (D-15) before this build existed, so whatever was observed in that position was not a tray. The row's substance — that the bar reads dense rather than simplified toward the reference shells, with both drawers and all four extras present — is unaffected and the `PASS` stands on that basis. See `## Post-Gate Correction`. |
...
| ID | Criterion (verbatim) | Baseline | Gesture | Verdict | Observation |
|---|---|---|---|---|---|
| B.1 | Every readout the three retired layouts collectively exposed (clock, battery when present, network, bluetooth, audio, cpu/ram/disk) is present and live on the new bar. | `config-full`, `config-floating`, `config-vertical` (bluetooth sourced from athena, per the Criterion B Index correction) | Walk the criterion IDs the index maps to B.1 and, for each, find its readout on the bar and confirm the value is live rather than static: put the CPU under load and watch the number move, toggle wifi off and on and watch the glyph change, play audio and watch the volume value follow. The battery entry rendering nothing on this batteryless desktop is D-18-06 satisfied and is recorded as such, not as a missing readout. | **PASS** | **Iteration 3, live gesture.** CPU under load showed the number move; wifi toggled off and on showed the glyph change; audio played and the volume value followed. Battery rendered nothing, recorded as D-18-06 satisfied on this batteryless desktop, not a missing readout. All B.1-mapped criterion IDs confirmed live rather than static. |
| B.2 | Clicking a workspace switches to it (the capability dead under waybar 0.15.0's compiled-in dispatch — QBAR-03's whole reason for existing). | `config-full`, `config-floating`, `config-vertical` (via the shared `hyprland/workspaces` dead dispatch) | Click a non-active occupied workspace slot, then click an **empty** slot, confirming both switch (check `hyprctl activeworkspace -j`). The empty slot is the specific trap 18-09 names: an empty workspace has no Hyprland object at all, and the object-only dispatch path can silently do nothing without anyone noticing if the gate only exercises occupied tiles. | **PASS** | **Iteration 3, live gesture.** Clicked a non-active occupied workspace slot and it switched; clicked an empty slot and it also switched — confirmed via `hyprctl activeworkspace -j`. The empty-slot trap 18-09 named (no Hyprland object, object-only dispatch silently doing nothing) did not reproduce. |
| B.3 | Scrolling on the audio section adjusts volume; scrolling on the brightness-bearing section adjusts brightness (parity with `config-floating`'s scroll bindings). | `config-floating` (audio scroll-step; brightness, D-18-39) | Two halves recorded in one cell. **Audio (live gesture):** scroll up and down over the audio entry and confirm `wpctl get-volume @DEFAULT_AUDIO_SINK@` moves with it — three wheel-up notches should raise the value by 15 percentage points, bound holding at both ends. **Brightness (transcribed, not judged — D-18-39):** copied verbatim from `18-SCROLL-GATE-RECORD.md` § "Section 1": verdict **"not demonstrable on this hardware — structurally present"**. Evidence: `/sys/class/backlight/` is empty (desktop board, no panel); `brightnessctl -m -l -c backlight` prints nothing and exits non-zero; `light`, which `config-floating.jsonc`'s own `"on-scroll-up": "light -A 5"` / `"on-scroll-down": "light -U 5"` shelled out to, is not installed — the capability being compared against was already dead in the baseline, per `18-BEHAVIOUR-BASELINE.md`'s own `## Dead Definitions` (`floating` / `backlight` row). This half is a transcription; forming a fresh judgment about it here is exactly what 18-12 wrote the sentence down in advance to prevent. | **NOT-DEMONSTRABLE** | **Iteration 3, both halves.** **Audio half (live gesture, met):** scrolled up and down over the audio entry; `wpctl get-volume @DEFAULT_AUDIO_SINK@` moved with it. **Brightness half (transcribed, not judged — D-18-39):** verdict **"not demonstrable on this hardware — structurally present"**, transcribed verbatim from `18-SCROLL-GATE-RECORD.md`. Evidence: `brightnessctl -m -l -c backlight` prints nothing and exits 1; `/sys/class/backlight/` is empty; `18-BEHAVIOUR-BASELINE.md`'s `## Dead Definitions` records the retired bar's own backlight scroll (`config-floating`'s `light` binding) as already dead. This row's verdict cell reads `NOT-DEMONSTRABLE` rather than `PASS` — B.3 is the single row D-18-39 authorises to carry it; its appearance on any other row would itself be a gate failure. `NOT-DEMONSTRABLE` is not a `FAIL` and does not block the deletion. |
| B.4 | The horizontal↔vertical toggle (settings drawer + Super-menu, D-18-30) reaches the vertical orientation and every readout present in horizontal is still present, in the 44px stacked-text form, with no truncation. | `config-vertical` | Flip to vertical from the settings drawer, then flip back and flip again from the Super-key menu's settings entry, confirming both reach paths work (D-18-30). In vertical, walk every readout counted in B.1 and confirm each is present in the 44px column in its stacked or abbreviated form with nothing clipped — the clock as two stacked lines, other readouts as glyph plus short value (D-18-14). The acceptance bar is no truncation, not an identical stacking shape everywhere. | **PASS** | **Iteration 3, live gesture.** Flipped to vertical from the settings drawer, flipped back, and flipped again from the Super-key menu's settings entry — both reach paths worked. In vertical, every readout counted in B.1 was present in the 44px column in stacked or abbreviated form with nothing clipped: clock as two stacked lines, other readouts as glyph plus short value. **Known-state notes:** (1) brightness/battery percent readouts overhang the 44px column by a measured ~3px (`right=47` vs `44`), named in this iteration's `## Build Under Test` block before the sitting and left unfixed (`Grid` sizes columns from `implicitWidth`, not resolved `width`). (2) `2644ae0` deliberately centre-bands workspaces/idle-inhibitor in vertical, a departure from UI-SPEC's Orientation Transform Rules ("no centre-banding" in vertical), made at the operator's own explicit request after being shown the rule. The operator judged the criterion sentence itself (every readout present, in the stacked/abbreviated form, with no truncation) met regardless: no readout is clipped or missing, the acceptance bar is no truncation rather than an identical stacking shape everywhere, and the centre-banding is an operator-directed layout choice, not a missing or truncated readout. |
| B.5 | Tray icons render and their menus open on click, in both orientations. | `config-full`, `config-vertical`, `config-floating` | In horizontal, confirm the tray capsule holds one cell per running tray application and that clicking one opens its menu anchored below; flip to vertical and repeat, confirming the menu anchors leftward (D-18-04). If the tray is empty, start a tray application first — the empty-tray collapse is a different, already-owned state and is not what this row tests. | **OVERRIDDEN** | **Reclassified post-gate from `PASS` — see `## Post-Gate Correction` below.** This row was authored from a UI-SPEC written before phase 18.1 withdrew the tray, and it could not have been observed as written: `TrayCapsule.qml` was deleted by 18.1-04 (D-15) and no `SystemTray` implementation exists anywhere under `quickshell/`. The capability is not lost — it is **deliberately withdrawn**, and `QBAR-05` carries `[W] Withdrawn` in REQUIREMENTS.md with the reason: athena's own config dropped the tray because nm-applet and blueman icons duplicated the connections group, which the connections capsule surfaces instead. Recorded as `OVERRIDDEN` rather than `PASS` so a withdrawn capability stays visible in the table instead of disappearing behind a green cell. |
| B.6 | Nothing deliberately cut during athena's own 08-16 evolution (e.g. the old tray-folded settings sub-menu) is expected back — only a genuine `full`/`floating`/`vertical`-exclusive capability counts as a regression if missing. | `config-full`, `config-floating`, `config-vertical` (exclusion list: `18-BEHAVIOUR-BASELINE.md` § `## Dead Definitions`) | Walk `18-BEHAVIOUR-BASELINE.md`'s `## Dead Definitions` table first and confirm nothing listed there (athena's `tray`, floating's `backlight`) is being counted as missing, then walk the remaining criterion IDs the index maps to B.6 and confirm each is either present on the bar or accounted for in that exclusion list. Only a genuine `full`/`floating`/`vertical`-exclusive capability counts as a regression. | **PASS** | **Iteration 3, live gesture.** Walked `18-BEHAVIOUR-BASELINE.md`'s `## Dead Definitions` table first: athena's `tray` (removed at the 08-16 checkpoint) and `config-floating`'s `backlight` scroll (dead for want of `light`) confirmed not counted as missing. Walked the remaining B.6-mapped criterion IDs: each was present on the bar or accounted for in the exclusion list. No genuine `full`/`floating`/`vertical`-exclusive capability found missing. |
...
- **B.5** — accepted as `OVERRIDDEN`, not `PASS`. Reason: the criterion audits tray parity against
  `config-full` / `config-vertical` / `config-floating`, all of which carried a tray, but the tray
  was **deliberately withdrawn** from the new bar by phase 18.1 plan 04 (D-15) — matching athena's
  own config, which removed it because nm-applet and blueman icons duplicated the `group/connections`
  entry. The connections capsule surfaces that state instead, so the capability is superseded rather
  than lost. Follow-up filed as `QBAR-05`, carrying `[W] Withdrawn` in `REQUIREMENTS.md` with the
  same rationale. Nothing is owed; this entry exists so the withdrawal is visible in the gate table
  rather than implied by a green cell.

## Post-Gate Correction

Recorded 2026-08-13, after Iteration 3 closed and after RETIRE-02 executed. This section is
**append-only and does not reopen the gate** — the deletion authorisation below stands, because the
correction removes a claim rather than adding a failure.

**What was wrong.** Rows `A.5` and `B.5` were transcribed verbatim from `18-UI-SPEC.md`, which was
written before phase 18.1 existed. 18.1 plan 04 then deleted `TrayCapsule.qml` (507 lines), its
`qmldir` row, its `Bar.qml` `componentFor()` branch and `trayComponent` block, and its
`BarEntryModel.qml` entry, under decision D-15. A repo-wide sweep confirms no `SystemTray` or
`DBusMenu` implementation remains anywhere under `quickshell/`. Both rows therefore asked the
operator to observe a component that had not existed in this codebase for a day, and both were
recorded as `PASS`.

**Why it was not caught.** The gate sheet's fifteen rows were locked at task 1 and verified for
*shape* — count, order, verbatim criterion text — by an automated check that had no way to ask
whether each criterion still had a subject. Every automated gate in this phase passed on this build.
This is the fifth purely visual or structural defect in phase 18 to survive a green automated pass,
and unlike F1–F5 it survived the human pass too, because a human is no better than the sheet they
are handed. **The lesson is narrower and sharper than "add another check": a gate sheet transcribed
from a spec must be re-validated against the codebase at each iteration's fingerprint, not only at
authoring time.** A criterion whose subject was removed between authoring and sitting is
indistinguishable, to the person sitting, from one that is simply passing.

**What it does and does not change.** It does not change the deletion authorisation: `QBAR-05` was
formally withdrawn with a written rationale before the gate ran, so no waybar capability was lost
unknowingly — the exact harm GATE-02 exists to prevent did not occur. It does change `B.5`'s verdict
from `PASS` to `OVERRIDDEN` (see `## Developer Overrides`) and annotates `A.5`'s stale tray clause,
so the record no longer asserts an observation that could not have been made. The tally for
Iteration 3 is therefore **13 PASS, 1 NOT-DEMONSTRABLE, 1 OVERRIDDEN, 0 FAIL** — not the
14/1/0/0 recorded at close.
...
The operator performed the sitting live, at the machine, against the fingerprint bound above:
HEAD `2644ae0563f2d330b8d615d355f523a44047da02`, quickshell pid `3012973`, on 2026-08-12. All
fifteen rows were observed in the fixed order — Block A, then Block B, then the three named
sub-judgments, then the lifted row — and each verdict and observation is written directly into
the master tables above (`## Block A`, `## Block B`, `## Named Sub-Judgments`, `## Lifted UI-SPEC
Rows Judged Here`); this list is the completed checklist, not a second copy of those tables.
...
- [x] B.1 — **PASS**
- [x] B.2 — **PASS**
- [x] B.3 — **NOT-DEMONSTRABLE** (audio half live and met; brightness half transcribed verbatim per D-18-39 — the single row authorised to carry this token)
- [x] B.4 — **PASS**
- [x] B.5 — **PASS**
- [x] B.6 — **PASS**
- [x] B.4-DRAWER — **PASS**
...
**Tally: 14 `PASS`, 1 `NOT-DEMONSTRABLE` (B.3), 0 `FAIL`, 0 `OVERRIDDEN`.** All fifteen rows carry
a verdict from the closed vocabulary; none was left at the placeholder.

**Four residuals survive the gate as accepted cosmetic debt, not as blockers.** The clock-pill
offset (A.1, ~6px), the workspace-numeral offset (A.3, ~3px), the percent-readout overhang (B.4,
~3px) and the slider-track offset (B.4-DRAWER, ~10px) were each named in this iteration's
`## Build Under Test` block before the sitting began, and each row's Observation cell records the
residual and that the operator judged the criterion sentence met on that known state, not on an
unexamined one. All four share one root cause — implicit-size inference in
`PopoutTrigger`/`Readout`-family components inducing a parent-child measurement cycle wherever a
fix tries to centre a child on a container that itself sizes from that child's `childrenRect` —
recorded in `260812-pd5-SUMMARY.md`. Closing them is a follow-up architectural task (the fix needs
a size the parent reserves explicitly, rather than one inferred from the child positioned against
it), not a GATE-02 blocker: none of the four rows failed, so none is written as a
`## Developer Overrides` entry — an override exists only for a row whose verdict would otherwise
be `FAIL`, and no row here was.

Iteration 2's single reported defect (F5, popout cards sitting low) is fixed, its remedy commits
(`cefcf20`, `7aa2cfd`, `d1acef4`, `23c7d21`) are named in this iteration's `## Build Under Test`
block, and Iteration 3's A.2/B.5/B.4-DRAWER observations above confirm no regression on the
surfaces F5 touched. `## Deletion Authorisation` below records the close.

**HEAD advanced by two docs-only commits after the fingerprint was bound, before this write-up.**
The fingerprint above binds this iteration to `2644ae0563f2d330b8d615d355f523a44047da02`, the sha
the operator actually observed. Between that capture and this write-up, two commits landed that
touch only `.planning/`: `caec5b9` (opening this Iteration 3 block) and `22a3d6b` (a STATE.md
checkpoint) — current HEAD immediately before this write-up's own commit is
`22a3d6b8a26d6feabeb6988a7411ceb71a4decf3`. Neither commit touches
`quickshell/.config/quickshell/`, confirmed: `git diff --name-only 2644ae0..22a3d6b --
quickshell/.config/quickshell/` returns empty. The build actually observed is therefore unchanged;
