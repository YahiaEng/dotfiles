...
key-decisions:
  ...
  - "A full uninterrupted 120s real-idle wall-clock observation could not be cleanly isolated on this live, actively-used desktop session -- three independent ~130s waits showed the compositor's own idle timer either correctly blocked by a genuine media-inhibit lock (first attempt, video playing) or correctly not firing at all (second/third attempts, real concurrent physical input on the shared session). This is evidence the idle detection is tracking genuine input correctly, not a broken listener. Verified the exact idle hide/show commands hypridle's on-timeout/on-resume invoke by direct CLI invocation instead -- the identical code path 08-03 already proved live -- and confirmed via hypridle's own startup log that the new 120s rule registered with exactly the intended on-timeout/on-resume commands."
...
- **Real 120s idle wall-clock observation was not cleanly isolatable.** Three independent ~130s waits were run. The first (with a YouTube video playing in `zen`) showed hypridle's internal idle timer correctly reaching 120s (an `Idled: rule ...` log line appeared) but the on-timeout command was correctly suppressed by the browser's screensaver-inhibit lock (`Playing video`) -- expected, unrelated to this plan's config. The second and third attempts (video paused) showed no `Idled` log line at all within the window, indicating genuine physical input occurred on this live, actively-used desktop session during the test (the shared machine has a real human user, per the executor's own live-session context). This is evidence the idle detection is working correctly (tracking real input, not broken), not a bug to fix. Resolved by verifying the exact `idle hide`/`idle show` CLI commands directly (the identical commands hypridle's `on-timeout`/`on-resume` invoke) across repeated iterations, plus confirming via hypridle's own startup log that the new listener rule registered with the exact intended commands.
