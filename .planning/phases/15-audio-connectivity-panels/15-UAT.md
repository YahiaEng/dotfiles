...
- gap_id: G-15-4
  ...
  user_decision: "DISPLACE nm-applet (user-chosen 2026-08-02). Suppress the autostart via a stowed ~/.config/autostart/nm-applet.desktop carrying Hidden=true — reproducible under stow, no host-only state. Do NOT build an out-of-band secret-agent process. This deliberately reverses part of 15-08's choice to install network-manager-applet; the package stays installed (blueman/nm-connection-editor tooling is unaffected), only its autostarted agent is suppressed. Because this leaves NO agent registered, the blind-spot measurement is now MANDATORY, not optional: the plan must determine what NM emits on a wrong PSK with no agent and which ConnectionFailReason reaches connectFailed, then make the row-scoped copy reachable for whatever that reason actually is."
...
- gap_id: G-15-4b
  ...
  user_decision: "ROUTE B PRIMARY, Route A as specified fallback (user-chosen 2026-08-02). Plan the directed-probe route (`nmcli device wifi rescan ssid <SSID>` → real Network object → existing connect flow unchanged, plus the mandatory `nmcli connection modify <name> 802-11-wireless.hidden yes` follow-up). Because no hidden SSID on this host is known, Route B cannot be proven up front — the plan must include an explicit verification step against a real hidden AP, and must specify Route A (with stdin+--ask so the passphrase never reaches argv) as the documented fallback if the AP does not answer directed probes. In scope for this gap-closure round; do NOT defer."
