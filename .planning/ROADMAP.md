...
- ✅ **v4.0 Shell Migration & Debt Paydown** — Phases 18-22 (shipped 2026-08-17) — [archive](milestones/v4.0-ROADMAP.md) · [summary](milestones/v4.0-MILESTONE.md)
...
### ✅ v4.0 Shell Migration & Debt Paydown (Phases 18-22) — SHIPPED 2026-08-17

One shell: waybar, swaync, swayosd, wleave and ags all replaced by QML surfaces and deleted from repo and host. 53/53 requirements, 63 plans, 567 commits. Full detail: [v4.0-ROADMAP.md](milestones/v4.0-ROADMAP.md) · [v4.0-MILESTONE.md](milestones/v4.0-MILESTONE.md)

<details>
<summary>Phase list</summary>
...
**Plans**: 10/10 plans executed, 7 waves

Plans:
**Wave 1**

- [x] 20-01-PLAN.md — GATE-01: live behaviour baseline, pre-deletion consumer sweep, and the three open-question measurements (wave 1)
- [x] 20-02-PLAN.md — LEDGER-05: WINDOWS.md triage at phase start against the live ledger (wave 1)
- [x] 20-03-PLAN.md — Shared declaration surface: 9 `Design.qml` tokens, `BarRoles.onWarn`, both new layer namespaces incl. the `quickshell-osd` `ignore_alpha` override (wave 1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 20-04-PLAN.md — OSD tracer: volume key → backend state → bottom-centre `Toast` instance, plus the full `swayosd-client` exec-target swap (wave 2)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 20-05-PLAN.md — OSD expansion: QOSD-04 recency-gated multi-slider column and QOSD-02 Caps Lock via a glob-resolved read-only sysfs watch (wave 3)
- [x] 20-06-PLAN.md — Power-menu tracer: 3×2 session dialog, all three entry points repointed, probe deleted, LEDGER-02 recorded (wave 3)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 20-07-PLAN.md — Power-menu expansion: QPOWER-03 detectors and banner, cascade entrance, cross-surface suppression (wave 4)
...
- [x] 20-09-PLAN.md — RETIRE-04: `swayosd` and its libinput backend removed, config-then-package (wave 6)

**Wave 7** *(blocked on Wave 6 completion)*

- [x] 20-10-PLAN.md — RETIRE-05 + RETIRE-07: `wleave` removed, then `wlogout` + `eww` in one `pacman -Rns` (wave 7)
...
**Plans**: 5/9 plans executed across 5 waves
**UI hint**: yes

- [x] 21-01-PLAN.md — TRACER: cava → QML streaming contract proven end-to-end on real audio, plus the reference-counted owner with a short linger and a one-knob always-on (QMEDIA-02)
