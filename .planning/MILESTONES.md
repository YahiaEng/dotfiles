...
- 6 unresolved debug sessions in `.planning/debug/` (5 `diagnosed`, 1 `unknown`): `bluetooth-enable-inert`, `panels-missing-animated-border`, `wifi-hidden-network-not-detected`, `wifi-hidden-network-unsupported`, `wifi-scan-progress-feedback`, `wifi-wrong-password-external-dialog`. Only `.planning/debug/resolved/` has been curated.
...
- WINDOWS #14: ~8 legacy `hyprctl dispatch global <name>` call sites in `quickshell-doctor` remain on the withdrawn string form, silently dead under the Lua config manager. Deferred by operator decision 2026-07-28. Caution: that script's headless-output add/remove test has previously SEGV-crashed the compositor during a DP-1 hotplug.
