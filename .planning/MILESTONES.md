...
**Requirements:** 39/40 satisfied at close (QS-03 formally dropped one-way under D-13; MAINT-02 unfinished) — **MAINT-02 has since been closed, 2026-08-27, by quick task 260827-74s**, so the standing count is now 40/40 with QS-03 dropped
...
- **MAINT-02 - Logout wrapping.** ✅ **CLOSED 2026-08-27** by quick task 260827-74s — the measurement was taken from the journal (`app-graphical.slice` released +67ms into a real teardown, envelope down at +593ms, with VSCodium/Zen/7 kitty live) and confirmed by a live Log Out: bare `uwsm stop` does not stall, so no wrap was needed. Original entry follows. The one genuinely unfinished requirement at close. 3 of 4 Phase 4 advisory items closed (fisher `-f`, nvm guard, uv env guard, all fault-injection proven, commit `baae579`); Logout stays on the bare path **by default, not by evidence** - its D-29 teardown measurement was waived by explicit operator decision on 2026-07-28 rather than performed, so the hazard is neither confirmed nor falsified. Reproduction steps remain verbatim in `13-03-PLAN.md` Task 2.
...
- **OVER-04's frame-rate term.** The CPU half was measured and passed with 2.4x headroom (20.9% worst of one core against a 50% ceiling); the FPS floor and target are recorded UNMEASURED and the verdict does not claim them - the only instrument for it froze the machine. **RESOLVED 2026-08-11 in Phase 18 under LEDGER-03 (`18-FRAME-RATE.md`):** measured with Qt's `QSG_RENDER_TIMING=1` instead of the compositor overlay, which was never retried. The **60 fps floor passes** — 0 of 81,261 render-loop iterations exceeded 16.67 ms during a human-driven overview drag at the load floor, worst case 12 ms (83.3 fps); the only sub-60 fps events found across ~89,000 measured frames were 11 single-frame bar-transition hitches (38-62 ms) and 2 overview surface-creation frames (90/93 ms). The **165 fps target is recorded as not resolvable** with that instrument and is deliberately not claimed as a pass.
