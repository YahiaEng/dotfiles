...
note (2026-08-01): you reported the bottom rounded corners as pixelated on the first build. Root-caused to the `MultiEffect` mask thresholding a texture rather than drawing a curve; two parameter fixes failed before the approach itself was replaced with real `Shape`/`CurveRenderer` ring geometry (`b80096d`). Antialiased pixels in the corner arc went 0.5% → 3.9%. Re-check the corners specifically when judging this item.
