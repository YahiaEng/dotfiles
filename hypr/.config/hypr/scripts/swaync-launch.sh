...
# (D-01/D-34/13-02) — mirrors waybar-launch.sh's shape rather than inlining
# a path into autostart.conf, which avoids relying on shell expansion
# inside a Hyprland exec-once line and gives the missing-sheet case
# somewhere to degrade.
#
...
# same discipline as waybar-launch.sh (a themed daemon is better than none,
# an unstyled daemon is better than none, "none" is the only real failure).
set -uo pipefail
...
if [[ -f "$COMPILED_STYLE" ]]; then
    ...
    # for waybar-launch.sh's own missing-sheet fallback).
