...
# ║  temp PATH and drives theme-switch.sh non-interactively   ║
# ║  to assert the three-way exit-code branch: walker 130 =   ║
# ║  silent cancel, other nonzero = loud failure, 0 = normal  ║
# ║  selection flow.                                           ║
# ╚══════════════════════════════════════════════════════╝
#
# Usage: test-walker-dmenu-cancel.sh [theme-switch]
#   With no argument: runs the theme-switch suite.
#
# Report-only — never mutates real theme state; walker and notify-send
# are shimmed on a prepended PATH, and the theme-switch success case runs
# under a fully isolated temp HOME.
#
...
case "${1:-theme-switch}" in
    theme-switch) run_theme_switch_suite ;;
    *)
        echo "Usage: $(basename -- "$0") [theme-switch]" >&2
        exit 2
        ;;
esac
